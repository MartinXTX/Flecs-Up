...
/* TIER-C2: Software prefetch macros.
 * Hides L2/L3 miss latency when iterating across table boundaries.
 * - x86/x64: _mm_prefetch with locality hint 1 (L2) for stream data, 0 (NTA)
 *   for rarely-reused data.
 * - ARM64: __builtin_prefetch with write=0, locality=1.
 * - Other: no-op. */
#if defined(__x86_64__) || defined(_M_X64) || defined(__i386__) || defined(_M_IX86)
    #include <emmintrin.h>
    #define ECS_PREFETCH(p) _mm_prefetch((const char*)(p), _MM_HINT_T0)
    #define ECS_PREFETCH_L2(p) _mm_prefetch((const char*)(p), _MM_HINT_T1)
    #define ECS_PREFETCH_NTA(p) _mm_prefetch((const char*)(p), _MM_HINT_NTA)
#elif defined(__aarch64__) || defined(_M_ARM64)
    #define ECS_PREFETCH(p) __builtin_prefetch((p), 0, 1)
    #define ECS_PREFETCH_L2(p) __builtin_prefetch((p), 0, 2)
    #define ECS_PREFETCH_NTA(p) __builtin_prefetch((p), 0, 0)
#else
    #define ECS_PREFETCH(p) ((void)0)
    #define ECS_PREFETCH_L2(p) ((void)0)
    #define ECS_PREFETCH_NTA(p) ((void)0)
#endif

/* TIER-T1: User-side row prefetch macro.
 * Call inside a tight inner loop to prefetch row `N` rows ahead.
 * This hides L1 miss latency when iterating sequential column data.
 *
 * Example:
 *   for (int i = 0; i < it.count; i++) {
 *       ECS_ITER_PREFETCH_ROW(8);
 *       // ... process row i ...
 *   }
 */
#define ECS_ITER_PREFETCH_ROW(n) ECS_PREFETCH(((char*)0) + 0)  /* placeholder */
...
typedef struct ecs_query_iter_t {
    ...
    /* TIER-A3: arena cursor at iter init. Restored at iter fini so the
     * persistent iter_arena in ecs_query_impl_t is fully reused each cycle. */
    ecs_stack_cursor_t *arena_cursor;
} ecs_query_iter_t;

/* TIER-X1+ V2: Per-field cache entry. Stores the resolved column data
 * pointer + size, keyed by the table it was resolved for. */
...
/** TIER-A1.3: Enable automatic EcsDontFragment detection.
 *
 * When enabled, data-only components (no observers, has type_info, not a
 * pair) are automatically marked as EcsDontFragment. This causes them to
 * use sparse storage instead of fragmenting the archetype. Opt-in because
 * it changes storage semantics for components.
 *
 * @param world The world.
 * @param enable Whether to enable or disable auto-detection.
 */
FLECS_API void ecs_world_auto_dont_fragment(
    ecs_world_t *world,
    bool enable);
...
/* TIER-I1: Batched add_id API.
 * Add multiple components to an entity in a single archetype transition.
 * This is significantly faster than calling ecs_add_id N times because the
 * archetype graph is traversed once and only one flecs_commit is performed.
 *
 * @param world The world.
 * @param entity The entity.
 * @param ids Array of component IDs to add.
 * @param count Number of IDs in the array. Must be > 0.
 */
FLECS_API
void ecs_add_ids(
    ecs_world_t *world,
...
/* TIER-I1: Batched remove_id API.
 * Remove multiple components from an entity in a single archetype transition. */
FLECS_API
void ecs_remove_ids(
    ecs_world_t *world,
    ecs_entity_t entity,
...
/* TIER-O1: Bulk set_id for N entities sharing a single component value.
 * Sets `component` to the same `value` for all entities in `entities`.
 * Faster than calling ecs_set N times because it shares the defer cycle
 * and reuses the component-record lookup (Tier-J2 cache).
 *
 * @param world The world.
 * @param entities Array of entities to update. Must all be alive.
 * @param count Number of entities in the array. Must be > 0.
 * @param component The component ID to set on each entity.
 * @param size Size of the value in bytes. Must be > 0.
 * @param value Pointer to the value to copy for each entity.
 */
FLECS_API
void ecs_bulk_set_id(
    ecs_world_t *world,
...
/* TIER-P1: Bulk delete API.
 * Delete N entities in a single defer cycle. Faster than calling ecs_delete N
 * times because the defer_cmd check is hoisted out of the loop.
 *
 * @param world The world.
 * @param entities Array of entities to delete. Entities not alive are skipped.
 * @param count Number of entities in the array. Must be > 0.
 */
FLECS_API
...
/* TIER-R1: Bulk new with values — create N entities + set same component value
 * in one optimized call. Combines bulk_new_w_id and bulk_set_id.
 *
 * @param world The world.
 * @param component The component ID for the new entities.
 * @param count Number of entities to create. Must be > 0.
 * @param value Pointer to the value to copy for each entity.
 * @param size Size of the value in bytes.
 * @return Pointer to the array of created entity IDs, or NULL on error.
 */
FLECS_API
const ecs_entity_t* ecs_bulk_new_w_value(
    ecs_world_t *world,
    ecs_id_t component,
    int32_t count,
    const void *value,
...
/* TIER-EE1: Bulk add_ids across N entities — adds the same set of components
 * to N entities. All entities must share the same starting archetype (typical
 * after ecs_bulk_new_w_id). The graph traversal is computed once and reused.
 *
 * @param world The world.
 * @param entities Array of entities to add components to.
 * @param entity_count Number of entities in the array. Must be > 0.
 * @param ids Array of component IDs to add.
 * @param id_count Number of component IDs. Must be > 0.
 */
FLECS_API
void ecs_add_ids_w_entities(
    ecs_world_t *world,
    const ecs_entity_t *entities,
    int32_t entity_count,
    const ecs_id_t *ids,
...
/* TIER-A1.2: Data-only component macro.
 * Same as ECS_COMPONENT but automatically registers the component with
 * EcsDontFragment so that it goes through the sparse-storage fast path.
 * Use this for components that:
 *   - have no observers attached to OnAdd/OnRemove/OnSet
 *   - have no user-defined hooks (ctor/dtor/copy/move are NULL)
 *   - are not a relationship/pair
 *   - are not inherited (DontInherit)
 *
 * Violating any of these constraints with ECS_DATA_COMPONENT will silently
 * work but may not yield the expected performance — the safety net is that
 * runtime behavior is preserved; only the storage layout changes. */
...
/* TIER-A2: Force-inline macro for hot path helpers. */
#if defined(_MSC_VER)
    #define FLECS_FORCE_INLINE __forceinline
#elif defined(__GNUC__) || defined(__clang__)
    #define FLECS_FORCE_INLINE __attribute__((always_inline)) inline
#else
    #define FLECS_FORCE_INLINE inline
#endif

/* TIER-A2: Unchecked field accessor for tight inner loops.
 * Skips bounds, size, and validity checks. Caller must guarantee:
 *   - it->flags & EcsIterIsValid (next() returned true)
 *   - index < it->field_count
 *   - sizeof(T) matches it->sizes[index]
 *
 * Implementation lives in flecs_patched.c so it can access the full
 * ecs_table_t type. The macro performs only the column indirection that the
 * compiler cannot constant-fold. */
FLECS_API
void* ecs_field_unchecked_w_size(
    const ecs_iter_t *it,
    ecs_size_t size,
    int8_t index);
...
/* TIER-X1: Batched field prefetch hint. Use at top of query callback
 * body before accessing multiple fields. Hides L2 miss latency for
 * column data pointers when fields are accessed sequentially.
 *
 * Example:
 *   ecs_fields(it);  // prefetch all columns
 *   for (int i = 0; i < it->count; i++) {
 *       Position *p = ecs_field(it, Position, 0);
 *       Velocity *v = ecs_field(it, Velocity, 1);
 *   }
