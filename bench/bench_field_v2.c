/* Microbenchmark v2: ecs_field_w_size indirection overhead — fixed printf format. */
...
int main(void) {
    ...
    printf("[bench_field_v2] %.0f M calls | %.3f sec | %.2f ns/call | %.2f M/sec\n",
        calls / 1e6, elapsed_sec, ns_per_call, calls / elapsed_sec / 1e6);
